  ...
  <div class="modal-dialog  ">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title text-capitalize" id="exampleModalLabel ">Creating  Admin / Change Password</h5>
      ...
      <form id="formData">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" id="formSubmit">Register</button>
        </div>
      </form>
    </div>
  </div>
  ...
  $(document).ready(function(){
    ...
    function fill_datatable()
    {
      var dataTable = $('#list-table').DataTable({
        ...
        "ajax":{
            "url": "{{route('superadmin.users.show','dataTable')}}",
            "dataType": "json",
            "type": "GET",
            "data":{
      ...
      $('#formSubmit').on('click', function(){
        ...
        var tpay = $('#formAppend');
        var name = tpay.find('#name').val();
        var email = tpay.find('#email').val();
        var password = tpay.find('#password').val();
        if(name == ''){
           tpay.find('#error_name').html('name field is required!');
        }
        else if (email == '') {
          tpay.find('#error_email').html('email field is required!');
        }
        else if(password == ''){
          tpay.find('#error_password').html('password field is required!');
        }else{
...
$('#modelBtn').on('click', function(){
  var url = "{{ route('superadmin.users.create') }}";
  modalAppend(url);
});
