  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <!-- <button type="button"  class="btn btn-primary float-right text-capitalize " >
        ...
        <h1 class="text-center ">Inactive Gymer </h1>
      </div>
      <div class="col-sm-4">
  ...
  $(document).ready(function(){
    ...
    function fill_datatable()
    {
      var dataTable = $('#list-table').DataTable({
        ...
        "ajax":{
            "url": "{{route('superadmin.inactive_gymer.show','dataTable')}}",
            "dataType": "json",
            "type": "GET",
            "data":{
      ...
      $('#list-table tbody').on('click','.modalbtn, .active_gymer,.delete_gymer', function(event){
        ...
        if(formOf === 'active_gymer') {
          const url = $(this).attr('action');
          active_gymer(url, dataTable);
        }
...
$('#modelBtn').on('click', function(){
  var url = "{{ route('superadmin.gender.create') }}";
  modalAppend(url);
});
</script>
@endpush
